...
export default function ServicesPage() {
  return <>
    ...
    {cleaningOffers.map((offer, index) => <section key={offer.value} id={offer.value} className={'py-14 md:py-20 scroll-mt-24 ' + (index % 2 ? 'bg-brand-mist' : 'bg-white')}>
      <Reveal className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          <div className={'overflow-hidden rounded-lg group ' + (index % 2 ? 'lg:order-2' : '')}><CleaningPhoto photo={offer.photo} priority={index === 0} sizes="(min-width: 1024px) 50vw, 100vw" className="w-full aspect-[4/3] object-cover" /></div>
          <div>
            ...
            {offer.value === 'window-cleaning' && <p className="text-muted-foreground mb-6">Choose a one-off clean or a recurring schedule. Our <Link to="/blog/how-often-clean-commercial-windows-adelaide" className="text-brand-ink underline">guide to window-cleaning frequency</Link> explains what affects the timing. For hard-to-reach windows, we assess height and access before confirming the work.</p>}
    ...
    <section className="py-14 md:py-20 bg-primary/5 border-t border-primary/10 text-center"><div className="max-w-3xl mx-auto px-4"><h2 className="mb-5">Not sure where to start?</h2><p className="text-lg text-muted-foreground mb-7 mx-auto">Just tell us what needs cleaning. We’ll help you work out which service suits the job. You can also <Link to="/blog/regular-cleaning-vs-deep-cleaning-adelaide" className="text-brand-ink underline">compare regular cleaning and deep cleaning</Link> before requesting a quote.</p><Button asChild size="lg"><Link to="/#find-your-clean">Help me choose a clean</Link></Button></div></section>
  </>;
}
