...
import { cleaningOffers, cleaningPhotos, cleaningPlans, windowAccessOptions, cleaningQuotePath } from '@/data/cleaningExperience.js';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <Tabs value={service} onValueChange={value => { setService(value); setPlan(value === 'commercial' ? 'regular' : 'one-off'); }}>
        ...
        {cleaningOffers.map(offer => <TabsContent key={offer.value} value={offer.value} className="m-0 service-panel">
          <div className="grid lg:grid-cols-2 overflow-hidden rounded-md bg-white border border-slate-200 ">
            <div className="relative min-h-60 lg:min-h-[490px] overflow-hidden">
              ...
              <div className="absolute inset-x-0 bottom-0 px-5 py-3 border-t border-slate-200 bg-white/95 text-slate-700"><span className="text-sm font-semibold">{cleaningPhotos[offer.photo].caption || (['builders', 'retail'].includes(offer.photo) ? 'From a MisterClean job in Hindmarsh.' : 'Service illustration')}</span></div>
            </div>
            <div className="p-6 sm:p-8 lg:p-10">
              ...
              <div className="flex flex-wrap gap-3 mt-6"><Button asChild size="lg" className="h-auto min-h-12 whitespace-normal"><Link to={cleaningQuotePath(service, plan, access)}>Get a quote for this clean <ArrowRight className="h-4 w-4 ml-2 shrink-0" /></Link></Button><Button asChild variant="outline" size="lg" className="min-h-12"><Link to={offer.path}>Service details</Link></Button></div>
              <p className="text-sm text-muted-foreground mt-4">You can add more details on the next page.</p>
            </div>
          </div>
        </TabsContent>)}
      </Tabs>
