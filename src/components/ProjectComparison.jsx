...
export default function ProjectComparison({ onOpen }) {
  const id = useId();
  ...
  return <div className="rounded-lg overflow-hidden border border-slate-200 bg-white">
    ...
    <div id={id} className={view === 'both' ? 'grid sm:grid-cols-2' : 'max-w-3xl mx-auto'}>
      <figure hidden={view === 'after'}><ProjectPhoto photo="kitchenBefore" onOpen={onOpen} className="w-full h-auto" /><figcaption className="px-4 py-3 text-sm"><strong>Before</strong><span className="block text-slate-600 mt-1">Dust and loose materials on the floor.</span></figcaption></figure>
      <figure hidden={view === 'before'}><ProjectPhoto photo="kitchenAfter" onOpen={onOpen} className="w-full h-auto" /><figcaption className="px-4 py-3 text-sm"><strong>After floor cleaning</strong><span className="block text-slate-600 mt-1">Fit-out still in progress; equipment remains wrapped.</span></figcaption></figure>
    </div>
